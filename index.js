...
const PLUGIN_NAME = 'gulp-avif-webp-retina-html';
 ...
 * @param {Boolean} [options.noAvif=false] If true, disables generate <source> tag with avif images
...
module.exports = function(options) {
  ...
  const addAvif = (!options.noAvif && options.extensions.length > 0);
    ...
    try {
      ...
      const pictureRender = (mainImage, fallbackImage) => {
        ...
        if (addAvif) {
            ...
            let avifFiePath = `${ mainImgName }.avif`;

            if (fileExists(avifFiePath)) {
                imgset.get('avif').push(avifFiePath);
              }
        }
      ...
      data = data.map(function(line) {
        ...
        if (~line.indexOf('<img') && !inPicture) {
          ...
          if (options.extensions.includes(mainImage['imgExt']) && (addAvif || addWebp || addRetina)) {
